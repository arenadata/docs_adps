...
+ Изменение URL-адреса репозитория YUM (см. п. 1.1);
+ Ввод имен хостов и SSH-ключа (см. п. 3.1);
+ Ручная установка Ambari-агентов (см. п. 2.4.1);
+ Выбор сервисов (см. п. 3.2);
+ Назначение мастер-компонентов (см. п. 3.3);
+ Назначение Slave и Client (см. п. 3.4);
+ Дополнительные настройки сервисов (см. п. 2.5).



#.

    #. 1.1.Изменение URL-адреса репозитория YUM
...
установленных репозиториев стека (см. п. 2.1).
...
#.
    ...
    #. 3.1.Ввод имен хостов и SSH-ключа
...
скопировать и вставить его содержимое в экранную форму (см. п. 1.1.1).
...
#.
    ...
    #.
        ...
        #. 2.4.1.Ручная установка Ambari-агентов
...
возможно установить позднее (Рис. 5.).
...
#.
    ...
    #. 3.3.Назначение мастер-компонентов



Необходимо назначить мастер-компоненты для узлов кластера (Рис. 6.).
...
#.
    ...
    #. 2.5.Дополнительные настройки сервисов
...
#.
    ...
    #. 2.6.Установка, запуск и тестирование
...
Если в процессе настройки сервера Ambari (см. п. 2.3) необходимо
