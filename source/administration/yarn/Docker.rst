...
Для запуска приложения в Docker-контейнере необходимо установить следующие переменные среды в среде приложения:
...
``YARN_CONTAINER_RUNTIME_DOCKER_IMAGE`` -- имена, образ которых будет использоваться для запуска Docker-контейнера. Можно использовать любое имя образа, которое можно передать команде запуска Docker-клиента. Имя образа может включать repo-префикс;
