...
+ `Настройка сопоставления групп Hadoop для LDAP/AD с использованием SSSD`_ (рекомендуется)
+ `Настройка сопоставления групп Hadoop в файле core-site.xml`_
+ `Ручное создание пользователей и групп в среде Linux`_


Настройка сопоставления групп Hadoop для LDAP/AD с использованием SSSD
```````````````````````````````````````````````````````````````````````
