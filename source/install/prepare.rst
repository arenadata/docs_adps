...
Ambari 2.2.1 совместим с версиями ADH 1.3 и ADH 1.4.
...
максимального количества открытых файловых дескрипторов».1

.. list-table:: Таблица 1. Минимальные системные требования для установки кластера Arenadata Ambari
   :header-rows: 1   
   ...
   * - База данных
     - Ambari требует реляционной базы данных для хранения информации о
       ...
       пункте «Настройка сервера Ambari 2.2.1» (см. п. 2.3)
       ...
       новую базу данных доступен только в сообществе
...
.. list-table:: Таблица 2. Требования к объему памяти в зависимости от размера кластера
   :header-rows: 1
...
.. list-table:: Таблица 3. Приблизительные значения размера пакета и количества индексных дескрипторов
   :header-rows: 1
...
+ Проверить полное доменное имя каждого хоста в системе. Мастер
...
Развертывание всех компонентов ADH на одном хосте возможно, но это
подходит только для начальной установки. Как правило, для минимального
кластера настраивается три узла – один главный хост и два подчиненных.
...
+ Узел NameNode;
+ Узел DataNodes;
+ Вторичный узел NameNode;
+ Узел Оози;
+ Узел YARN;
+ Узел ZooKeeper, если устанавливается ZooKeeper;
+ Различные журналы, файлы pid и db, в зависимости от типа установки.

Необходимо использовать базовые каталоги, которые предоставляют
...
Для корректной развертки Hadoop необходимо ознакомиться и выполнить рекомендации:
...
Для настройки беспарольного SSH необходимо под учетной записью
администратора выполнить следующие инструкции:
...
+ Сохранить копию приватного ключа SSH на компьютере, с которого
...
Для каждого сервиса ADH необходима учетная запись пользователя. Мастер
...
Чтобы настроить службу NTP на автоматический запуск при загрузке
...
Чтобы запустить службу NTP, необходимо выполнить команду на каждом
хосте:
...
Все хосты в системе должны быть настроены как для прямого, так и для
обратного DNS.
...
адрес и доменное имя Fully Qualified Domain Name (FQDN) каждого хоста.
...
Работа с SELinux не поддерживается поэтому необходимо отключить SELinux:

Для этого следует установить значение
SELINUX=disabled в /etc/selinux/config и перезагрузить узел.
...
SLES этот шаг можно пропустить.
