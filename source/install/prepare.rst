...
даны материалы, которые необходимо подготовить для установки кластера (:term:`кластер`)
...
кластера **ADH**. Используя :term:`API-интерфейс` **Ambari Web UI** и **REST**, можно
развертывать, разрабатывать, управлять изменениями конфигурации и
контролировать службы всех узлов (:term:`узел`) кластера с центральной точки.
...
.. list-table:: Табл. 1. Минимальные системные требования для установки кластера Arenadata Ambari
   ...
   * - Операционная система
     - 
        ...
        репозиториев ОС, доступных для всех компьютеров во время |br| 
        установки, могут возникнуть проблемы. |br|
        
        В случае возникновения проблем с недоступными базовыми |br|
        хранилищами ОС, необходимо обратиться к системному |br|
        администратору для обеспечения проксирования или |br|
        зеркалирования этих дополнительных репозиториев |br| 

   * - Браузер
     - 
       Мастер установки Ambari работает как веб-приложение на |br|
       основе браузера. Поэтому необходимо наличие машины, |br|
       способной использовать графический браузер для |br| 
       применения данного веб-инструмента.
       ...
       последней, стабильной версии (кроме Internet Explorer 9.0) |br| 

   * - Программное обеспечение
     - На всех узлах кластера необходимо установить следующие |br| 
       компоненты:
   ...
   * - Java
     - Поддерживаются следующие среды выполнения Java:
       ...
       Во время установки необходимо выбрать JDK 1.8, поскольку |br| 
       ADH 1.4 несовместим с JDK 1.7 |br| 
   ...
   * - База данных
     - Ambari требует реляционную базу данных для хранения |br| 
       ...
       По умолчанию Ambari устанавливает инстанс PostgreSQL на |br| 
       хост сервера Ambari. При этом использование данного |br| 
       инстанса PostgreSQL, MySQL или Oracle необязательно. |br| 
       Дополнительная информация указана в пункте «Настройка |br| 
       сервера Ambari 2.5.1»
       ...
       По умолчанию Ambari устанавливает инстанс Derby на хосте |br| 
       Oozie Server. При этом использование данного инстанса |br| 
       PostgreSQL, MySQL или Oracle необязательно. Использование |br| 
       Derby в качестве базы метаданных Oozie не рекомендуется |br| 
       для продуктивного использования. Дополнительная |br| 
       информация указана в пункте «Настройка сервера Ambari |br| 
       2.5.1».

       Инстанс Derby для рабочей среды не должен использоваться |br| 
       по умолчанию. Если планируется применение Derby для |br| 
       demo-версии, разработки или тестирования окружающей  |br| 
       среды, перенос базы данных Oozie из Derby в новую базу  |br| 
       данных доступен только в Community Edition
       ...
       Для Ranger необходим указанный инстанс PostgreSQL, MySQL |br| 
       или Oracle.
       
       Для базы данных Ambari, если используется существующая |br| 
       база данных Oracle, необходимо убедиться, что инстанс |br| 
       Oracle работает на порте, отличном от 8080. Это |br| 
       необходимо, чтобы избежать конфликта с портом Ambari, |br| 
       установленном по умолчанию. Также следует обратиться к |br| 
       "Справочному руководству Ambari" для получения информации |br| 
       о возможных изменениях порта по умолчанию сервера Ambari.
       
       Использование параметров базы данных Microsoft SQL Server |br| 
       или SQL Anywhere не поддерживается.
       
       Если используется существующая база данных, развернутая на |br| 
       том же сервере, что и Ambari Server, необходимо, чтобы эта |br| 
       база работала на порте, отличном от 8080 |br| 
...
:term:`Хост` **Ambari** должен иметь как минимум *1 ГБ* оперативной памяти и
...
Рекомендуемое максимальное количество открытых файловых дескрипторов (:term:`файловый дескриптор`) –
...
+ Проверить :term:`полное доменное имя (FQDN)` каждого хоста в кластере. Мастер
  установки Ambari поддерживает использование IP-адресов (:term:`IP-адрес`). Для проверки и
  установки FQDN необходимо воспользоваться командой:

    :command:`hostname –f`

Развертывание всех компонентов (:term:`компонент`) **ADH** на одном хосте возможно, но не рекомендуется для продуктивного использования. Как правило, для минимального кластера настраивается три узла – один главный и два подчиненных.
...
+ Установить базовые каталоги, которые будут определены в качестве точек для хранения:
  ...
  + Узла :term:`NameNode`;
  + Узла (узлов) :term:`DataNode` (в этих каталогах не должно содержаться никаких других данных);
...
+ Настройка беспарольного :term:`SSH` доступа к хостам;
...
Для :term:`Ambari Server` необходимо, чтобы :term:`Ambari Agents` были установлены на
...
сервером **Ambari** необходимо для учетной записи, под которой производится установка (по умолчанию от :term:`root`), настроить беспарольное SSH-соединение между хостом **Ambari Server** и всеми другими хостами в кластере.
...
Хост сервера **Ambari** использует аутентификацию (:term:`аутентификация`) публичного ключа SSH для
...
файл */etc/hosts* на каждом узле кластера так, чтобы он содержал :term:`IP-
адрес` и :term:`полное доменное имя` (**FQDN**) каждого узла.
