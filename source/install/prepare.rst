...
.. list-table:: Табл. 1. Минимальные системные требования для установки кластера Arenadata Ambari
   ...
   * - Операционная система
     - 
        ...
        зеркалирования этих дополнительных репозиториев
   ...
   * - Браузер
     - 
       ...
       На платформе рекомендуется обновить браузер до последней, |br| 
       стабильной версии (кроме Internet Explorer 9.0)
   ...
   * - База данных
     - Ambari требует реляционную базу данных для хранения |br| 
       ...
       для продуктивного использования. Дополнительная информация |br| 
       указана в пункте «Настройка сервера Ambari 2.5.1».

       Инстанс Derby для рабочей среды не должен использоваться |br| 
       по умолчанию. Если планируется применение Derby для |br| 
       demo-версии, разработки или тестирования окружающей среды, |br| 
       перенос базы данных Oozie из Derby в новую базу данных |br| 
       доступен только в Community Edition
       ...
       Для базы данных Ambari, если используется существующая база |br| 
       данных Oracle, необходимо убедиться, что инстанс Oracle |br| 
       работает на порте, отличном от 8080. Это необходимо, чтобы |br| 
       избежать конфликта с портом Ambari, установленном по |br| 
       умолчанию. Также следует обратиться к "Справочному |br| 
       руководству Ambari" для получения информации о возможных |br|
       изменениях порта по умолчанию сервера Ambari.
       
       Использование параметров базы данных Microsoft SQL Server или |br| 
       SQL Anywhere не поддерживается.
       
       Если используется существующая база данных, развернутая на |br| 
       том же сервере, что и Ambari Server, необходимо, чтобы эта |br| 
       база работала на порте, отличном от 8080
