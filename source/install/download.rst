...
Для загрузки дистрибутива (:term:`дистрибутив`) кластера (:term:`кластер`) **Arenadata Hadoop** (далее – **ADH**) необходимо на сайте продукта http://arenadata.io/index.html перейти на вкладку "Скачать". При этом открывается страница *ARENADATA.STORE* (:ref:`Рис.1.<install-img-1>`):
...
Окно сообщения следует закрыть. При этом происходит переход на страницу с выбором компонентов (:term:`компонент`) дистрибутива (:ref:`Рис.5.<install_pic5>`).
...
+ *Arenadata Hadoop 1.4* – RPM-пакеты (:term:`RPM-пакет`) для сервисов Hadoop, таких как :term:`HDFS`, :term:`YARN`, :term:`Hbase`, Hive, Zookeeper;
